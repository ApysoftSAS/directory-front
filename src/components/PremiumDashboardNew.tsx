...
import { Promotion, Review, BusinessStats, Business, PromotionalBanner, BusinessReport } from '../types';
...
interface PremiumDashboardNewProps {
  businessId?: string;
}

const generateMockReportData = (): BusinessReport => {
  const data = [];
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    data.push({
      ...
      whatsappClicks: Math.floor(Math.random() * 30) + 10,
      websiteClicks: Math.floor(Math.random() * 20) + 5,
      phoneClicks: Math.floor(Math.random() * 15) + 5,
    });
  }
...
export const PremiumDashboardNew: React.FC<PremiumDashboardNewProps> = ({ businessId = '1' }) => {
  ...
  const [reportData] = useState<BusinessReport>(generateMockReportData());
  ...
  const [reviews, setReviews] = useState<Review[]>([
    ...
    },
    {
      ...
      rating: 5,
      comment: 'Productos de calidad y entrega rápida',
      verified: true,
      approved: false,
    ...
    },
    {
      ...
      rating: 3,
      comment: 'El servicio estuvo bien pero el tiempo de espera fue largo',
      verified: false,
      approved: false,
  ...
  const handleApproveReview = (reviewId: string) => {
    ...
    toast.success('Opinión aprobada exitosamente');
  };

  const handleRejectReview = (reviewId: string) => {
    setReviews(reviews.filter((r) => r.id !== reviewId));
    toast.success('Opinión rechazada y eliminada');
  };
  ...
  const exportReport = () => {
    const csvContent = [
      ['Fecha', 'Vistas', 'Clics', 'WhatsApp', 'Sitio Web', 'Teléfono'],
    ...
    a.href = url;
    a.download = `reporte-${business?.name}-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  const totalViews = reportData.data.reduce((sum, item) => sum + item.views, 0);
  const totalClicks = reportData.data.reduce((sum, item) => sum + item.clicks, 0);
          ...
          <TabsContent value="analytics">
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h3 className="text-xl">Reportes y Estadísticas</h3>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <Card className="p-6">
                  <h4 className="text-lg mb-4">Vistas y Clics (30 días)</h4>
                  <ResponsiveContainer width="100%" height={250}>
                    <LineChart data={reportData.data}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="views" stroke="#2563eb" name="Vistas" />
                ...
                <Card className="p-6">
                  <h4 className="text-lg mb-4">Interacciones por Tipo</h4>
                  <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={reportData.data.slice(-7)}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis 
                        dataKey="date" 
                      ...
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="whatsappClicks" fill="#25D366" name="WhatsApp" />
                      <Bar dataKey="websiteClicks" fill="#10b981" name="Sitio Web" />
                      <Bar dataKey="phoneClicks" fill="#6366f1" name="Teléfono" />
                    </BarChart>
                  </ResponsiveContainer>
                </Card>
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-gray-600">Total Vistas (30d)</p>
                      <p className="text-3xl mt-2">{totalViews.toLocaleString()}</p>
                    </div>
                ...
                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-gray-600">Total Clics (30d)</p>
                      <p className="text-3xl mt-2">{totalClicks.toLocaleString()}</p>
                    </div>
                ...
                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl mt-2">{((totalClicks / totalViews) * 100).toFixed(1)}%</p>
                    </div>
                  ...
                  <Card key={promo.id} className="p-4 sm:p-6">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          {promo.active ? <CheckCircle className="w-4 h-4 mr-1" /> : null}
          ...
          <TabsContent value="reviews">
            <Card className="p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                  <h3 className="text-lg sm:text-xl mb-2">Gestión de Opiniones</h3>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-base sm:text-lg">
                      {avgRating.toFixed(1)} ({reviews.filter(r => r.approved).length} aprobadas)
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    <option value="pending">Pendientes ({reviews.filter(r => !r.approved).length})</option>
                    <option value="approved">Aprobadas ({reviews.filter(r => r.approved).length})</option>
                  </select>
                </div>
              </div>
              ...
              <div className="space-y-6">
                {reviews
                  .filter((review) => {
                    if (reviewFilter === 'pending') return !review.approved;
                  ...
                  <div key={review.id} className="border rounded-lg p-4 bg-white">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-3">
                      <div className="flex items-start gap-3">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-indigo-600 flex items-center justify-center text-white flex-shrink-0">
                    ...
                    <p className="text-sm sm:text-base text-gray-700 mb-3 pl-13">{review.comment}</p>
                    
                    {review.response && (
                      <div className="ml-13 bg-gray-50 border-l-4 border-blue-500 p-3 rounded-r mb-3">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <div className="flex items-center gap-2">
                            <Reply className="w-4 h-4 text-blue-600" />
                            <span className="text-sm text-blue-600">Respuesta del negocio</span>
                          </div>
                          <Button
                            size="sm"
                            variant="ghost"
                      ...
                      <div className="ml-13">
                        {replyingTo === review.id ? (
                          <div className="bg-blue-50 border border-blue-200 rounded p-3">
                            <Label className="text-sm mb-2 block">Responder a {review.userName}</Label>
                            <Textarea
                              value={replyText}
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500">
                      {reviewFilter === 'pending' 
                        ? 'No hay opiniones pendientes de aprobación'
